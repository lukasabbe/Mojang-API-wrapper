...
class MinecraftProfile {    MinecraftUUID;
    MinecraftUsername;
    MinecraftSkinData;
    OptifineCapeUrl;
    constructor(MinecraftUUID, MinecraftUsername){
...
const functions = {
    ...
    checkIfServerBlocked: async (serverIp) => {
        const data = await fetch(`https://sessionserver.mojang.com/blockedservers`);
        if(!data.ok) return false;
        const textData = await data.text();
    
        const lines = textData.split("\n");
        let hash = await crypto.subtle.digest("SHA-1", serverIp);
    ...
    getOptfineCapeUrl: async (username) => {
